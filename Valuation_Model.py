...
def growth_path(g1_used: float, g_terminal: float, years: int, constant_growth: bool):
    if years <= 0 or g1_used is None or g_terminal is None:
        return []

    if constant_growth:
        return [g1_used for _ in range(years)]

    return [
        g1_used + (g_terminal - g1_used) * (i / (years - 1)) if years > 1 else g1_used
        for i in range(years)
    ]


def clamp_g_terminal_by_currency(g_terminal: float, currency_code: str | None):
    warnings = []
    if g_terminal is None:
        return None, warnings

    g_terminal_used = g_terminal
    if currency_code == "EUR" and g_terminal_used > 0.02:
        g_terminal_used = 0.02
        warnings.append("g terminal clampé à 2% (EUR).")
    elif currency_code == "USD" and g_terminal_used > 0.025:
        g_terminal_used = 0.025
        warnings.append("g terminal clampé à 2.5% (USD).")

    return g_terminal_used, warnings
...
def compute_fcf_start_from_history(hist_df: pd.DataFrame, allow_negative: bool):
    ...
    details["used_values"] = used_values
...
def build_fcf_projection(
    ...
    growths = growth_path(g1_used, g_terminal, years, constant_growth)
...
def build_tornado_table(
    ...
    scenarios = [
        ("Croissance +0.5%", g1_used + 0.005, wacc, g_terminal, fcf_start),
        ("Croissance -0.5%", g1_used - 0.005, wacc, g_terminal, fcf_start),
        ("WACC +0.5%", g1_used, wacc + 0.005, g_terminal, fcf_start),
        ("WACC -0.5%", g1_used, max(0.0001, wacc - 0.005), g_terminal, fcf_start),
        ("g terminal +0.25%", g1_used, wacc, g_terminal + 0.0025, fcf_start),
        ("g terminal -0.25%", g1_used, wacc, max(0.0, g_terminal - 0.0025), fcf_start),
        ("FCF_start +10%", g1_used, wacc, g_terminal, fcf_start * 1.10 if fcf_start is not None else None),
        ("FCF_start -10%", g1_used, wacc, g_terminal, fcf_start * 0.90 if fcf_start is not None else None),
    ]
...
def analyze_company(
    ...
    # =========================
    g_terminal_used, g_terminal_warnings = clamp_g_terminal_by_currency(g_terminal, currency_code)
    dcf_allowed, dcf_missing, dcf_warnings = validate_inputs_and_data(
        ...
        net_debt=net_debt,
        wacc_used=wacc_used,
        g_terminal=g_terminal_used,
        allow_negative_fcf=allow_negative_fcf,
    )
    dcf_warnings.extend(g_terminal_warnings)
    if fcf_details:
        dcf_warnings.extend(fcf_details.get("warnings", []))
    if wacc_used is not None and g_terminal_used is not None and (wacc_used - g_terminal_used) < 0.02:
    ...
    if dcf_allowed:
        ...
        fv_base = fv_dcf
        fv_low, _, _, _, _, _, _, _, _, _, _, _ = compute_dcf_valuation(
            fcf_start=fcf_start,
            g1_used=g1_used,
            years=years,
            wacc=wacc_used + 0.01,
        ...
        if wacc_used is not None and g_terminal_used is not None and (wacc_used - 0.005) > g_terminal_used:
            fv_high, _, _, _, _, _, _, _, _, _, _, _ = compute_dcf_valuation(
                fcf_start=fcf_start,
                g1_used=g1_used,
                years=years,
                wacc=max(0.0001, wacc_used - 0.005),
                g_terminal=g_terminal_used,
                net_debt=net_debt,
                shares=shares,
                constant_growth=constant_growth,
        ...
        if price not in (None, 0) and fv_base is not None:
            mos = (fv_base - price) / price
            if mos >= 0.25:
                verdict = "Sous-valorisé"
            elif mos >= -0.10:
                verdict = "Juste prix"
            else:
                verdict = "Sur-valorisé"

        if ev not in (None, 0) and tv_discounted is not None:
            tv_weight = tv_discounted / ev
        else:
            tv_weight = None
        implied_ev_fcf_terminal = None
        if wacc_used is not None and g_terminal_used is not None and wacc_used > g_terminal_used:
            implied_ev_fcf_terminal = (1 + g_terminal_used) / (wacc_used - g_terminal_used)
        if tv_weight is not None and tv_weight > 0.80:
            dcf_warnings.append("TV > 80% de l'EV (sensibilité élevée).")
        if implied_ev_fcf_terminal is not None and implied_ev_fcf_terminal > 25:
            dcf_warnings.append("EV/FCF terminal implicite > 25x (agressif).")
    else:
    ...
    dcf_audit = {
        "fcf_start": fcf_start,
        "fcf_start_method": (fcf_details or {}).get("method"),
        "years_used": (fcf_details or {}).get("years_used", []),
        ...
        "tv_weight": tv_weight,
        "implied_ev_fcf_terminal": implied_ev_fcf_terminal,
...
def main():
    ...
    def render_wacc_auto_inputs():
        ...
        return {
            ...
            "beta_sector_override": beta_sector_override,
            "beta_override": beta_override,
            "rd_override_pct": rd_override_pct,
            "override_tax": override_tax,
            "tax_override_pct": tax_override_pct,
            "allow_heuristic_rd": allow_heuristic_rd,
    ...
    growth_fcf_input = 3.0
    constant_growth = False
    allow_negative_fcf = False
    prudence_hist = True
    g_terminal_input = 1.75
    exit_method = "Gordon"
    ...
    with col4:
        st.metric("Prix de marché", safe_metric(result.get("price"), "{:.2f}"))
        if dcf_active:
            st.metric("Juste valeur DCF", safe_metric(dcf.get("fair_value_per_share"), "{:.2f}"))
        else:
...
# ----- TAB 1 : Résumé DCF -----
    with tab_resume:
        ...
        if not dcf_active:
            ...
            with col_c:
                st.metric("Juste valeur / action", safe_metric(fair_value_per_share, "{:,.2f}"))
                st.metric("Nombre d'actions", format_large_number(shares))

            audit = result.get("dcf_audit", {}) or {}
            g_input = audit.get("g_input")
            g1_used = audit.get("g1_used")
                ...
                for warning in warnings:
                    ...
                    if "EV/FCF terminal implicite > 25x" in warning:
                        explanations.append(
                            ...
                            "Multiple élevé = hypothèses terminales ambitieuses."
                        )
                return explanations

            mos = result.get("mos")
            verdict = result.get("verdict", "N/A")
            fv_base = result.get("fv_base")
            fv_low = result.get("fv_low")
            fv_high = result.get("fv_high")
            ...
            if mode_ui == "Simple":
                ...
                if any("TV > 80% de l'EV" in w for w in dcf_warnings):
                    important_warnings.append("TV > 80% de l'EV")
                if any("EV/FCF terminal implicite > 25x" in w for w in dcf_warnings):
                    important_warnings.append("EV/FCF terminal implicite > 25x")
                if (
                    audit.get("wacc_used") is not None
                    and g_terminal_used is not None
                    and (audit.get("wacc_used") - g_terminal_used) < 0.025
                ):
                    important_warnings.append("WACC - g terminal < 2.5% (sensibilité accrue).")
                important_warnings = important_warnings[:3]
                ...
                st.write(f"- FCF de départ estimé : **{format_large_number(fcf_start)}**")
                st.write(
                    f"- Verdict : **{verdict}** (MOS : **{safe_metric(mos * 100, '{:.1f}', 'N/A')} %**)"
                )
                if dcf_warnings:
                    st.warning("Avertissements DCF :\n- " + "\n- ".join(map(str, dcf_warnings)))
                    with st.expander("Explications des avertissements"):
                        explanations = warning_explanations(dcf_warnings)
                        if explanations:
                            st.info("Explications :\n- " + "\n- ".join(explanations))
    ...
    with tab_proj:
        ...
        if (not dcf_active) or (proj_df is None) or proj_df.empty:
            ...
            st.markdown(
                ...
                f"{growth_mode_label} (g1 = **{safe_metric((result.get('dcf_audit', {}).get('g1_used') * 100 if result.get('dcf_audit', {}).get('g1_used') is not None else None), '{:.2f}')} %**, "
                f"g terminal = **{safe_metric((result.get('dcf_audit', {}).get('g_terminal_used') * 100 if result.get('dcf_audit', {}).get('g_terminal_used') is not None else None), '{:.2f}')} %**)."
            )
    ...
    with tab_dcf:
        ...
        st.write(f"- g base : **{safe_metric(g_terminal_input, '{:.2f}')} %**")
        st.write(f"- Croissance FCF : **{safe_metric(growth_fcf_input, '{:.2f}')} %/an**")
