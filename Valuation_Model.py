...
def get_net_debt(fundamentals: dict):
    ...
    try:
        ...
        net_debt = _get_float("netdebt", "NetDebt".lower())
...
if raw:
    ...
    # 2) Benchmark sectorisé (auto + cache), seulement si activé
    if use_sector_benchmark:
        sector = (company or {}).get("Sector")
        exch = (company or {}).get("Exchange")
        try:
            cached_dist, meta = load_sector_cache(sector, exch, sector_peers_limit)
        except Exception:
        ...
        if needs_refresh:
            ...
            if peers:
                try:
                    dist = compute_peer_distribution(peers, api_key)
                    benchmark_meta = {
                        "sector": sector,
                        "exchange": exch,
                        "limit": int(sector_peers_limit),
                        "peers_count": len(peers),
                        "updated_at_iso": datetime.utcnow().isoformat(timespec="seconds"),
                    }
                    save_sector_cache(sector, exch, sector_peers_limit, dist, benchmark_meta)
